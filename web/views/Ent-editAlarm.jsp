        ...
        <div class="admin-content-body">
            <div class="am-cf am-padding am-padding-bottom-0">
                <div class="am-fl am-cf">
                    <strong class="am-text-primary am-text-lg">编辑警报</strong> /
                    <small>Edit Alarm</small>
                </div>
            </div>
            ...
            <div class="am-g">
                ...
                <script>
                    $(function () {
                        //默认选择数据模板和数据点 select
                        $("select[name='iDatatemplateId']").val(${alarm.dataTemplate.id});
                        $("select[name='iDatapointId']").val(${alarm.dataPoint.id});
                        $("select[name='iDeviceId']").val(${alarm.device.id});
                        //默认报警条件 select
                        $("select[name='sTriggerCondition']").val(${alarm.sTriggerCondition});
                        //默认联系人 select
                        $("select[name='iContactId']").val(${alarm.contact.id});
                        //默认选择存储方式 radio
                        $("input:radio[name='iInsertType'][value='${alarm.iInsertType}']").attr("checked",'checked');
                        //默认是否启用 radio
                        $("input:radio[name='iStatus'][value='${alarm.iStatus}']").attr("checked",'checked');
                        //默认报警方式
                        <c:forEach var="li" items="${alarm.alarmModes}">
                            $("input:checkbox[name='alarmModes'][value='${li}']").attr("checked",'checked');
                        </c:forEach>
                    });
                </script>

                <div class="am-u-sm-12 am-u-md-8 am-u-md-pull-4">
                    <form class="am-form am-form-horizontal" action="${pageContext.request.contextPath}/Alarm/doEditAlarm" method="post">
                        <div class="am-form-group">
                            <label for="sTriggerName" class="am-u-sm-3 am-form-label">名称<span style="color: red;">*</span></label>
                            <div class="am-u-sm-9">
                                <input type="hidden" name="AlarmPK" value="${alarm.id}">
                                <input type="text" name="sTriggerName" id="sTriggerName" value="${alarm.sTriggerName}" placeholder="名称" required>
                            ...
                            $(function () {
                                ...
                                $("#sTriggerCondition").change(function () {
                                    ...
                                    if(value==2 || value==6 || value==3){
                                        ...
                                        $("#sTriggerConditionContent").append('<input type="text" name="iMin" value="${alarm.iMin}" placeholder="A" style="width: 40%;display: inline-block;" required>');
                                    }else if(value==4 || value==5){
                                        $("input[name='iMax']").remove();
                                        $("input[name='iMin']").remove();
                                        $("#sTriggerCondition").css({"width": "30%","display":"inline-block"});
                                        $("#sTriggerConditionContent").append('<input type="text" name="iMin" value="${alarm.iMin}" placeholder="A" style="width: 30%;display: inline-block;" required>\n' +
                                            '                                <input type="text" name="iMax" value="${alarm.iMax}" placeholder="B" style="width: 30%;display: inline-block;" required>');
                                    }else{
                                        $("input[name='iMax']").remove();
                                        $("input[name='iMin']").remove();
                        ...
                        <div class="am-form-group">
                            <label for="sAbnormalContent" class="am-u-sm-3 am-form-label">报警推送内容<span
                                    style="color: red;">*</span></label>
                            <div class="am-u-sm-9">
                                <textarea class="" rows="5" name="sAbnormalContent" id="sAbnormalContent" placeholder="触发警报时推送的消息" required>${alarm.sAbnormalContent}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="am-form-group">
                            <label for="sNomalContent" class="am-u-sm-3 am-form-label">恢复正常推送内容<span
                                    style="color: red;">*</span></label>
                            <div class="am-u-sm-9">
                                <textarea class="" rows="5" name="sNomalContent" id="sNomalContent" placeholder="警报解除时推送的消息" required>${alarm.sNomalContent}</textarea>
                            </div>
                        </div>
